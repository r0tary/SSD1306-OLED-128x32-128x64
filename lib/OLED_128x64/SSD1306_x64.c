...
unsigned char ssd1306_buffer[1024];        //64*32/8. Every bit is a pixel
uint8_t grid;
...
void SSD1306_setup(void){   
    ...
    SSD1306_command(0x12);//could depend on manufacturer/hardware layout
...
void SSD1306_update(){
    ...
    SSD1306_command(7);                 //last page
    
    
    unsigned short pixel_count = 1024;//128*32/8      
...
void SSD1306_clear(){
    memset(ssd1306_buffer,0,1024);//make every bit a 0
}
